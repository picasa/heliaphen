...
data_model_expansion <- reactive({
  ...
  nest() %>% 
  mutate(
    model=map(data, possibly(model_expansion, data.frame(NULL))),
    prediction=map2(list_ftsw, model, possibly(add_predictions, data.frame(NULL))),
    coefficients=map(model, possibly(tidy, data.frame(NULL))),
    rmse=map2(model, data, possibly(rmse, NULL))
  )
})
...
data_model_transpiration <- reactive({
  data_response_transpiration() %>% 
    ...
    nest() %>% 
    mutate(
      model=map(data, possibly(model_transpiration, data.frame(NULL))),
    prediction=map2(list_ftsw, model, possibly(add_predictions, data.frame(NULL))),
    coefficients=map(model, possibly(tidy, data.frame(NULL))),
    rmse=map2(model, data, possibly(rmse, NULL))
    )
