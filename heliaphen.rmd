...
renderPlotly({
  ggplotly(
    data_weight() %>%
      ...
      ggplot(aes(x=time, y=weight, color=treatment, group=cultivar, label=plant_code)) +
...
renderPlotly({
  ggplotly(
    data_ftsw() %>%
      mutate(cultivar=interaction(genotype, treatment, sep=" - ")) %>% 
      ggplot(aes(x=time, y=FTSW, color=treatment, group=cultivar, label=plant_code)) +
      geom_point(alpha=0.3) +
      geom_line(stat="smooth", method="loess", alpha=0.8, size=1) +
